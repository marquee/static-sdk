    ...
    function _traverse (node/*: ExpandedDescriptorNode*/, parent_path/*: string */) {
        ...
        if (null != node_path && 'function' === typeof node_path) {
            ...
            if (null == node_path_string) {
                throw new Error(`${ null != node_name ? node_name : 'UNNAMED' } path iteratee did not return a string, got null`)
            } else if ('string' !== typeof node_path_string) {
                throw new Error(`${ null != node_name ? node_name : 'UNNAMED' } path iteratee did not return a string, got ${ typeof node_path_string }: ${ node_path_string }`)
            }
